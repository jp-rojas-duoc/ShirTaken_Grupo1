...
class RepositorioPolerasRoom(context: Context) {
    ...
    suspend fun estaVacio(): Boolean = (dao.observarCatalogo().firstOrNull()?.isEmpty() ?: true)

    suspend fun poblarSiVacio() {
        if (estaVacio()) {
            val base = listOf(
                cl.shirtaken.shirtaken_grupo1.model.Polera(0,"Polera básica","ShirTaken",9990,"M","Negro","https://picsum.photos/300?1", conStock = true, esFavorita = false),
                cl.shirtaken.shirtaken_grupo1.model.Polera(0,"Polera logo","ShirTaken",12990,"L","Blanco","https://picsum.photos/300?2", conStock = true, esFavorita = false),
                cl.shirtaken.shirtaken_grupo1.model.Polera(0,"Polera premium","ShirTaken",15990,"S","Azul","https://picsum.photos/300?3", conStock = true, esFavorita = true)
            )
